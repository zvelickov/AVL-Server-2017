...
namespace Taxi.Communication.Server.Utils
{
    public class ReservationProcessingThread
    {
        ...
        private TList<Reservations> getPendingReservations()
        {
            ...
            try
            {
                retVal = DataRepository.ReservationsProvider.GetPending(m_numberOfRetries);
                ...
                if (retVal != null)
                {
                    ...
                    tl[4] = typeof(GisStreets);

                    DataRepository.ReservationsProvider.DeepLoad(retVal, true, DeepLoadType.IncludeChildren, tl);
                }
