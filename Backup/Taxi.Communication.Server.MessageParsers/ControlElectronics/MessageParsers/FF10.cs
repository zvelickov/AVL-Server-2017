...
{
    class FF10 : IGeneralMessageHandler
    {
        ...
        {
            if ((msg.MessageIndicator + msg.Command) == "FF10")
                return true;
            else
                return false;
        }
        ...
        public ParserResponseContainer ProcessMessage(DeviceMessage msg)
        {
            ...
            try
            {
                ...
                /*
                if ((retVal.currentStateString == "StateBusy" && retVal.CurrentPhoneCall != null) ||
                        (retVal.currentStateString == "StateIdle" && retVal.IDLastPhoneCall != 0))
                {
                    //ServiceCallBack.log.Debug("-------------------------------------------------------------------------");
                    //ServiceCallBack.log.Debug("ZORAN: Imav CurrentPhoneCall.IdPhoneCall = " + retVal.CurrentPhoneCall.IdPhoneCall.ToString());

                    long tmpIdPhoneCall = 0;

                    if (retVal.currentStateString == "StateBusy")
                        tmpIdPhoneCall = retVal.CurrentPhoneCall.IdPhoneCall;
                    else
                        tmpIdPhoneCall = (long)retVal.IDLastPhoneCall;

                    TList<Orders> tmpOrders = DataRepository.OrdersProvider.GetByIdPhoneCall(tmpIdPhoneCall);

                    if (tmpOrders != null && tmpOrders.Count > 0)
                    {
                        tFiscalReceipt.IdOrder = (long)tmpOrders[0].IdOrder;
                    }
                }
                */
            ...
            {
                log4net.LogManager.GetLogger("MyService").Error("Greska vo obrabotka  FF10 za Vozilo", ex);
                return null;
            }
