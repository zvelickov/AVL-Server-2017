...
namespace Taxi.Communication.Server
{
    ...
    public class AdministrationService : IAdministrationService
    {
        ...
        public Clients searchVipClients(ClientPhoneNumbers clientPhoneNumber)
        {
            ...
            try
            {                
                TList<ClientPhoneNumbers> phoneNumbers = DataRepository.ClientPhoneNumbersProvider.GetAll();
                ClientPhoneNumbers mClientPhoneNumber = null;

                if (phoneNumbers != null && phoneNumbers.Count > 0)
                {
                    log.Debug("ClientPhoneNumbersProvider.GetAll() = " + phoneNumbers.Count.ToString());
                    log.Debug("clientPhoneNumber.PhoneNumber.Substring (clientPhoneNumber.PhoneNumber.Length - 8) = " + clientPhoneNumber.PhoneNumber.Substring(clientPhoneNumber.PhoneNumber.Length - 8));

                    foreach (ClientPhoneNumbers cpn in phoneNumbers)
                    {
                        log.Debug("cpn.PhoneNumber.Substring(cpn.PhoneNumber.Length - 8) = " + cpn.PhoneNumber.Substring(cpn.PhoneNumber.Length - 8));

                        if (clientPhoneNumber.PhoneNumber.Substring(clientPhoneNumber.PhoneNumber.Length - 8) == cpn.PhoneNumber.Substring(cpn.PhoneNumber.Length - 8))
                        {
                            mClientPhoneNumber = cpn;
                            break;
                        }
                    }
                }
                else
                    log.Debug("ClientPhoneNumbersProvider.GetAll() vrati NULL !!!");

                if (mClientPhoneNumber != null)
                {
                    retval = DataRepository.ClientsProvider.GetByIdClient(mClientPhoneNumber.IdClient);

                    if (retval != null)
                        log.Debug("retval.Name = " + retval.Name);
                    else
                        log.Debug("retval = NULL");
                }                    
            }
            catch (Exception e)
            {
